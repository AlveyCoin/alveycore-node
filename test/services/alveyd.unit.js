...
describe('Alvey Service', function() {
	...
	var baseConfig = {
		node: {
			network: alveycore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		...
		it('will init caches', function() {
			...
			should.exist(alveyd.balanceCache);
			should.exist(alveyd.summaryCache);
			should.exist(alveyd.transactionDetailedCache);
			should.exist(alveyd.accountInfo);

			should.exist(alveyd.transactionCache);
			should.exist(alveyd.rawTransactionCache);
			should.exist(alveyd.rawJsonTransactionCache);
			should.exist(alveyd.blockCache);
			should.exist(alveyd.blockJsonCache);
			should.exist(alveyd.blockSubsidyCache);
			should.exist(alveyd.rawBlockCache);
			should.exist(alveyd.blockHeaderCache);
			should.exist(alveyd.zmqKnownTransactions);
			should.exist(alveyd.zmqKnownBlocks);
			should.exist(alveyd.lastTip);
			should.exist(alveyd.lastTipTimeout);
			should.equal(alveyd.dgpInfoCache, null, 'should be null');
			should.equal(alveyd.miningInfoCache, null, 'should be null');
			should.equal(alveyd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(alveyd.maxTxids, 1000);
			should.equal(alveyd.maxTransactionHistory, 50);
			should.equal(alveyd.maxAddressesQuery, 10000);
			should.equal(alveyd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(alveyd.spawnRestartTime, 5000);
			should.equal(alveyd.spawnStopTime, 10000);

			// try all interval
			should.equal(alveyd.tryAllInterval, 1000);
			should.equal(alveyd.startRetryInterval, 5000);

			// rpc limits
			should.equal(alveyd.transactionConcurrency, 5);
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			...
			alveyd.transactionConcurrency.should.equal(5);
		});
	});
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			...
			events[0].name.should.equal('alveyd/rawtransaction');
			events[0].scope.should.equal(alveyd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('alveyd/hashblock');
			events[1].scope.should.equal(alveyd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('alveyd/addresstxid');
			events[2].scope.should.equal(alveyd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('alveyd/addressbalance');
			events[3].scope.should.equal(alveyd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			...
			alveyd.subscribe.args[0][0].should.equal('rawtransaction');
			alveyd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			alveyd.unsubscribe.args[0][0].should.equal('rawtransaction');
			alveyd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			alveyd.subscribe.args[1][0].should.equal('hashblock');
			alveyd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			alveyd.unsubscribe.args[1][0].should.equal('hashblock');
			alveyd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			...
			var emitter2 = {};
			alveyd.subscribe('rawtransaction', emitter2);
			alveyd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			alveyd.subscribe('hashblock', emitter1);
			alveyd.subscribe('hashblock', emitter2);
			alveyd.subscribe('hashblock', emitter3);
			alveyd.subscribe('hashblock', emitter4);
			alveyd.subscribe('hashblock', emitter5);
			alveyd.subscriptions.hashblock.length.should.equal(5);

			alveyd.unsubscribe('hashblock', emitter3);
			alveyd.subscriptions.hashblock.length.should.equal(4);
			alveyd.subscriptions.hashblock[0].should.equal(emitter1);
			alveyd.subscriptions.hashblock[1].should.equal(emitter2);
			alveyd.subscriptions.hashblock[2].should.equal(emitter4);
		...
		it('will not remove item an already unsubscribed item', function() {
			...
			alveyd.unsubscribe('hashblock', emitter3);
			alveyd.subscriptions.hashblock.length.should.equal(1);
			alveyd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will handle multiple address subscribers', function() {
			...
			alveyd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			alveyd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			alveyd.unsubscribeAddress(emitter1);
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		...
		it('will remove empty addresses', function() {
			...
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(alveyd, 'unsubscribeAddressAll');
			alveyd.unsubscribeAddress(emitter1);
			alveyd.unsubscribeAddressAll.callCount.should.equal(1);
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			alveyd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			alveyd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			alveyd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			alveyd.unsubscribeAddressAll(emitter1);
			alveyd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			alveyd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			alveyd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(alveyd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a alvey.conf file', function() {
			...
			sinon.spy(alveyd, '_getDefaultConf')
			sinon.spy(alveyd, '_getNetworkConfigPath');
			sinon.spy(alveyd, '_parseBitcoinConf');

			var node = {};
			alveyd._loadSpawnConfiguration(node);

			alveyd._expandRelativeDatadir.callCount.should.equal(1);
			alveyd._checkConfigIndexes.callCount.should.equal(1);
			alveyd._getDefaultConf.callCount.should.equal(1);
			alveyd._getNetworkConfigPath.callCount.should.equal(1);
			alveyd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(alveyd.spawn.config);
			alveyd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: alveycore.Networks.testnet,
					configPath: '/tmp/.alveycore/alveycore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			alveyd.options.spawn.datadir = './data';
			var node = {};
			alveyd._loadSpawnConfiguration(node);
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the alvey.conf file', function() {
			...
			var node = {};
			(function() {
				alveyd._checkConfigIndexes(config, node);
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				alveyd.transactionDetailedCache.set(keys[i], {});
				alveyd.utxosCache.set(keys[i], {});
				alveyd.accountInfo.set(keys[i], {});
				alveyd.txidsCache.set(keys[i], {});
				alveyd.balanceCache.set(keys[i], {});
				alveyd.summaryCache.set(keys[i], {});
				alveyd.blockOverviewCache.set(keys[i], {});
				alveyd.blockJsonCache.set(keys[i], {});
			}
			alveyd._resetCaches();
			should.equal(alveyd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(alveyd.utxosCache.get(keys[0]), undefined);
			should.equal(alveyd.accountInfo.get(keys[0]), undefined);
			should.equal(alveyd.txidsCache.get(keys[0]), undefined);
			should.equal(alveyd.balanceCache.get(keys[0]), undefined);
			should.equal(alveyd.summaryCache.get(keys[0]), undefined);
			should.equal(alveyd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(alveyd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			alveyd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				alveyd.nodes[0].client.getInfo.callCount.should.equal(1);
				alveyd.nodes[1].client.getInfo.callCount.should.equal(1);
				alveyd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			alveyd._tryAllClients(function(client, next) {
				...
				alveyd.nodes[1].client.getInfo.callCount.should.equal(1);
				alveyd.nodes[2].client.getInfo.callCount.should.equal(1);
				alveyd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			alveyd._initChain(function() {
				log.info.callCount.should.equal(1);
				alveyd.getRawBlock.callCount.should.equal(1);
				alveyd.getRawBlock.args[0][0].should.equal('genesishash');
				alveyd.height.should.equal(5000);
				alveyd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			alveyd._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			alveyd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			alveyd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			alveyd.on('block', function(block) {
		...
		it('will not emit same block twice', function(done) {
			...
			alveyd.on('block', function(block) {
				block.should.equal(message);
				alveyd.emit.callCount.should.equal(1);
				alveyd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			alveyd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			alveyd._zmqBlockHandler(node, message);
			alveyd._rapidProtectedUpdateTip.callCount.should.equal(1);
			alveyd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
		...
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(alveyd, 'emit');
			var emitter = new EventEmitter();
			alveyd.subscriptions.hashblock.push(emitter);
			emitter.on('alveyd/hashblock', function(blockHash) {
				alveyd._rapidProtectedUpdateTip.callCount.should.equal(1);
				alveyd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				alveyd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			...
			alveyd.on('error', function(err) {
				...
				alveyd._resetCaches.callCount.should.equal(1);
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using alveycore.Transaction', function() {
			...
			var outputAddress = alveycore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = alveycore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: alveycore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = alveyd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addInput(alveycore.Transaction.Input({
				...
				script: alveycore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: alveycore.Script('OP_TRUE'),
					satoshis: 5000000000
		...
		it('will return unique values', function() {
			...
			tx.addOutput(alveycore.Transaction.Output({
				script: alveycore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(alveycore.Transaction.Output({
				script: alveycore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			var emitter = new EventEmitter();
			alveyd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('alveyd/addresstxid', function(data) {
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			emitter.on('alveyd/rawtransaction', function(hex) {
		...
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(alveyd, 'emit');
			alveyd.subscriptions.rawtransaction.push(emitter);
			emitter.on('alveyd/rawtransaction', function() {
				done();
			});
			var node = {};
			alveyd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			...
			alveyd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				alveyd._updateTip.callCount.should.equal(11);
				alveyd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			alveyd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			...
			node.zmqSubSocket.on('message', function() {
				alveyd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			...
			alveyd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				alveyd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			alveyd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				alveyd._zmqBlockHandler.callCount.should.equal(0);
				alveyd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			alveyd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			alveyd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			alveyd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			alveyd.on('tip', function(height) {
				...
				alveyd.height.should.equal(100);
				done();
			});
			alveyd._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			alveyd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				alveyd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			alveyd._process.kill = sinon.stub().throws(error2);
			alveyd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			...
			alveyd._spawnChildProcess(function(err) {
				alveyd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			...
			alveyd._spawnChildProcess(function(err) {
				alveyd._loadSpawnConfiguration.callCount.should.equal(1);
				alveyd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			alveyd._loadSpawnConfiguration = sinon.stub();
			alveyd.spawn = {};
			alveyd.spawn.exec = 'testexec';
			alveyd.spawn.configPath = 'testdir/alvey.conf';
			alveyd.spawn.datadir = 'testdir';
			alveyd.spawn.config = {};
			alveyd.spawn.config.rpcport = 20001;
			alveyd.spawn.config.rpcuser = 'alvey';
			alveyd.spawn.config.rpcpassword = 'password';
			alveyd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			alveyd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			alveyd._initZmqSubSocket = sinon.stub();
			alveyd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			alveyd._checkReindex = sinon.stub().callsArgWith(1, null);
			alveyd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/alvey.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				alveyd._loadTipFromNode.callCount.should.equal(1);
				alveyd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(alveyd._initZmqSubSocket.args[0][0].client);
				alveyd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				alveyd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(alveyd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
		...
		it('will respawn alveyd spawned process', function(done) {
			...
			alveyd.spawn.datadir = '/tmp/alvey';
			alveyd.spawn.configPath = '/tmp/alvey/alvey.conf';
			alveyd.spawn.config = {};
			alveyd.spawnRestartTime = 1;
			alveyd._loadTipFromNode = sinon.stub().callsArg(1);
			alveyd._initZmqSubSocket = sinon.stub();
			alveyd._checkReindex = sinon.stub().callsArg(1);
			alveyd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			alveyd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			...
			alveyd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						alveyd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will NOT respawn alveyd spawned process if shutting down', function(done) {
			...
			alveyd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						alveyd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			alveyd._spawnChildProcess(function(err) {
				alveyd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			alveyd._initZmqSubSocket = sinon.stub();
			alveyd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			alveyd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			alveyd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to alveyd/);
				alveyd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			alveyd.startRetryInterval = 1;
			var config = {};
			alveyd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				alveyd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			alveyd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			alveyd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				alveyd._loadTipFromNode.callCount.should.equal(1);
				alveyd._initZmqSubSocket.callCount.should.equal(1);
				alveyd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error from connectProcess', function(done) {
			...
			alveyd.start(function(err) {
				alveyd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			...
			alveyd.start(function(err) {
				should.not.exist(err);
				alveyd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(alveyd, '_spawnChildProcess');
			var nodes = [{}];
			alveyd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			alveyd.options = {
				connect: [
					{}
				]
			...
			alveyd.start(function(err) {
				should.not.exist(err);
				alveyd._initChain.callCount.should.equal(1);
				alveyd._connectProcess.callCount.should.equal(1);
				alveyd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.syncPercentage(function(err) {
				should.exist(err);
				alveyd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			alveyd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				alveyd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			...
			var options = {};
			alveyd.getAddressBalance(address, options, function(err) {
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			alveyd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				alveyd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					alveyd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			alveyd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = alveyd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = alveyd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = alveyd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = alveyd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				alveyd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		...
		it('will give rpc error from mempool query', function() {
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			alveyd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				alveyd._getHeightRangeQuery.callCount.should.equal(1);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			alveyd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				alveyd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			alveyd.getAddressTxids(address, options, function(err, txids) {
				...
				alveyd.getAddressTxids(address, options, function(err, txids) {
					...
					alveyd._getHeightRangeQuery.callCount.should.equal(2);
					alveyd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			alveyd.getAddressTxids(address, options, function(err, txids) {
				...
				alveyd.getAddressTxids(address, options, function(err, txids) {
					...
					alveyd._getHeightRangeQuery.callCount.should.equal(4);
					alveyd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			alveyd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				alveyd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					alveyd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						alveyd._getHeightRangeQuery.callCount.should.equal(3);
						alveyd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			var details = alveyd._getAddressDetailsForTransaction(tx, addresses);
			...
			alveyd._getAddressDetailsForInput.callCount.should.equal(1);
			alveyd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			alveyd.height = 300;
			var addresses = {};
			alveyd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			alveyd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				alveyd.getDetailedTransaction.callCount.should.equal(1);
				alveyd._getAddressDetailsForTransaction.callCount.should.equal(1);
				alveyd._getConfirmationsDetail.callCount.should.equal(1);
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from alveycore addresses', function() {
			var addresses = [
	...
	describe('#getAddressHistory', function() {
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			...
			alveyd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			alveyd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			alveyd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			alveyd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd._getAddressStrings.callCount.should.equal(1);
				alveyd.getAddressTxids.callCount.should.equal(1);
				alveyd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			alveyd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			alveyd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			alveyd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(alveyd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			alveyd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			alveyd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd._getAddressStrings.callCount.should.equal(1);
				alveyd.getAddressTxids.callCount.should.equal(1);
				alveyd._getAddressDetailedTransaction.callCount.should.equal(2);
				alveyd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			alveyd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			alveyd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			alveyd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			alveyd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			alveyd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			alveyd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			alveyd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			alveyd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			alveyd.getAddressSummary(address, options, function(err, summary) {
				alveyd._paginateTxids.callCount.should.equal(1);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd.getAddressTxids.callCount.should.equal(1);
				alveyd.getAddressBalance.callCount.should.equal(1);
				alveyd._paginateTxids.args[0][1].should.equal(0);
				alveyd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			alveyd.getAddressSummary(address, options, function(err, summary) {
				...
				alveyd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					alveyd._paginateTxids.callCount.should.equal(0);
					alveyd._normalizeAddressArg.callCount.should.equal(2);
					alveyd.getAddressTxids.callCount.should.equal(1);
					alveyd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			alveyd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				alveyd._paginateTxids.callCount.should.equal(1);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				alveyd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			alveyd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			alveyd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			alveyd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			alveyd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			alveyd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			alveyd.getBlock(0, function(err) {
				alveyd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as alveycore object from height', function(done) {
			...
			alveyd.getBlock(0, function(err, block) {
				should.not.exist(err);
				alveyd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(alveycore.Block);
				done();
			});
		});
		it('will getblock as alveycore object', function(done) {
			...
			alveyd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			alveyd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			alveyd.getBlock(0, function(err, block) {
				...
				alveyd.getBlock(0, function(err, block) {
					should.not.exist(err);
					alveyd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			alveyd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			...
			alveyd.getBlockHeader(10, function(err) {
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			alveyd.getBlockHeader(blockhash, function(err) {
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			alveyd.getBlockHeader(0, function(err) {
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			alveyd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			alveyd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			alveyd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				alveyd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			alveyd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			alveyd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will give expected result', function(done) {
			...
			alveyd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				alveyd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					alveyd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			alveyd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		...
		it('will give rpc error', function() {
			...
			alveyd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			alveyd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			alveyd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = alveycore.Transaction();
			(function() {
				alveyd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getRawTransaction('txid', function(err) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			alveyd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getTransaction('txid', function(err) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			alveyd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				alveyd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(alveycore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			alveyd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				alveyd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			alveyd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			alveyd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			alveyd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			...
			alveyd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			alveyd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			alveyd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				alveyd.getSubsidy.callCount.should.equal(1);
				alveyd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			alveyd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			...
			alveyd.stop(done);
			alveyd.spawn.process.kill.callCount.should.equal(1);
			alveyd.spawn.process.kill.args[0][0].should.equal('SIGINT');
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			alveyd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				alveyd._getBalanceFromMempool.callCount.should.equal(1);
				alveyd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			alveyd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			alveyd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			alveyd.getJsonBlock(0, function(err) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			alveyd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			alveyd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				alveyd._tryAllClients.callCount.should.equal(2);
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			alveyd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
				alveyd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will handle multiple address subscribers', function() {
			...
			alveyd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			alveyd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			alveyd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			alveyd.unsubscribeBalance(emitter1);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			alveyd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var alveyd = new AlveyService(baseConfig);
			...
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			alveyd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var alveyd = new AlveyService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(alveyd, 'unsubscribeBalanceAll');
			alveyd.unsubscribeBalance(emitter1);
			alveyd.unsubscribeBalanceAll.callCount.should.equal(1);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			alveyd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			alveyd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			alveyd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			alveyd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			alveyd.unsubscribeBalanceAll(emitter1);
			alveyd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			alveyd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			alveyd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(alveyd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				alveyd._notifyBalanceSubscriber.callCount.should.equal(1);
				alveyd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			alveyd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				alveyd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			alveyd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			alveyd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getDgpInfo(function(err, result) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getMiningInfo(function(err, result) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			alveyd.getStakingInfo(function(err, result) {
				should.exist(err);
				alveyd._tryAllClients.callCount.should.equal(1);

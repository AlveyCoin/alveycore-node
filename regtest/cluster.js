...
describe('Bitcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/alveyd');
  var nodesConf = [
    {
  ...
  before(function(done) {
    log.info('Starting 3 alveyd daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three alveyd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.alveyd.once('tip', function(height) {

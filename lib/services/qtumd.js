...
var bitcore = require('alveycore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var AlveyRPC = require('alveyd-rpc');
...
/**
 * Provides a friendly event driven API to alveyd in Node.js. Manages starting and
 * stopping alveyd as a child process for application support, as well as connecting
 * to multiple alveyd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Alvey(options) {
  if (!(this instanceof Alvey)) {
    return new Alvey(options);
  }
...
util.inherits(Alvey, Service);

Alvey.dependencies = [];

Alvey.DEFAULT_MAX_TXIDS = 1000;
Alvey.DEFAULT_MAX_HISTORY = 50;
Alvey.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Alvey.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Alvey.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Alvey.DEFAULT_SPAWN_RESTART_TIME = 5000;
Alvey.DEFAULT_SPAWN_STOP_TIME = 10000;
Alvey.DEFAULT_TRY_ALL_INTERVAL = 1000;
Alvey.DEFAULT_REINDEX_INTERVAL = 10000;
Alvey.DEFAULT_START_RETRY_INTERVAL = 5000;
Alvey.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Alvey.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Alvey.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Alvey.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Alvey.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Alvey.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Alvey.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Alvey.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Alvey.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Alvey.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Alvey.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Alvey.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Alvey.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Alvey.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Alvey.prototype._initCaches = function() {
...
Alvey.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Alvey.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Alvey.prototype.getPublishEvents = function() {
  return [
    {
...
Alvey.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'alveyd/' + name, 'total:', this.subscriptions[name].length);
};

Alvey.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'alveyd/' + name, 'total:', this.subscriptions[name].length);
};


Alvey.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Alvey.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Alvey.prototype.unsubscribeBalanceAll = function(emitter) {
...
Alvey.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'alveyd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Alvey.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'alveyd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Alvey.prototype.unsubscribeAddressAll = function(emitter) {
...
Alvey.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Alvey.DEFAULT_CONFIG_SETTINGS;
...
Alvey.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Alvey.prototype._expandRelativeDatadir = function() {
...
Alvey.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in alveyd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in alveyd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in alveyd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './alvey.conf');

  log.info('Using alvey config file:', configPath);
...
Alvey.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of alveycore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of alveycore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of alveycore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to alveyd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from alveyd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from alveyd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that alveyd is undergoing a reindex. ' +
             ...
             'of alveycore-node services will start.');
    node._reindex = true;
  }
};

Alvey.prototype._resetCaches = function() {
...
Alvey.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Alvey.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Alvey.prototype._initChain = function(callback) {
  var self = this;
...
Alvey.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Alvey.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/alvey.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/alvey.conf';
    }
  }
  return networkPath;
};

Alvey.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Alvey.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('alveyd/hashblock', message.toString('hex'));
    }
  }

};

Alvey.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Alvey.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Alvey Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Alvey.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Alvey.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Alvey.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Alvey.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('alveyd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Alvey.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('alveyd/rawtransaction', message.toString('hex'));
    }
...
Alvey.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Alvey.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Alvey.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Alvey.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Alvey.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Alvey Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Alvey.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Alvey.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Alvey.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/alveyd.pid';
...
Alvey.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting alvey process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Alvey process unexpectedly exited with code:', code);
        log.warn('Restarting alvey child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Alvey process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn alveyd.'));
      }
...
Alvey.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to alveyd.'));
    }
 ...
 */
Alvey.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Alvey configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Alvey.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Alvey.prototype.syncPercentage = function(callback) {
  var self = this;
...
Alvey.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, alveycore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Alvey.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Alvey.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Alvey.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Alvey.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in alveycore is most recent to less recent
 ...
 * @param {Function} callback
 */
Alvey.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Alvey.prototype._getConfirmationsDetail = function(transaction) {
...
Alvey.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Alvey.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Alvey.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A alveycoin transaction id
 * @param {Function} callback
 */
Alvey.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Alvey.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Alvey.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Alvey.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Alvey.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Alvey.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Alvey.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Alvey.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Alvey.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Alvey.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Alveycore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Alvey.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getInfo = function(callback) {
...
Alvey.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('alveyd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('alveyd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Alvey.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Alvey.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Alvey.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Alvey.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Alvey.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Alvey.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Alvey.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Alvey.prototype.getStakingInfo = function(callback) {
    var self = this,

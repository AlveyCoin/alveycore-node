...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for alveyd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and alveycore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed alveycore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './alveycore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in alveycore-node services directory
    ...
    if (servicePackage.alveycoreNode) {
      serviceModule = service.name + '/' + servicePackage.alveycoreNode;
    }

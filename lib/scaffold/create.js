...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'alveycore-lib': 'https://github.com/alveyproject/alveycore-lib.git#master',
    'alveycore-node': 'https://github.com/alveyproject/alveycore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/alveycore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the alveycore-node directory and configuration
